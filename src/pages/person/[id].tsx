...
const Person: NextPage = ({ person }: any) => {
  ...
  const truncText = <p>Оскар Айзек (Oscar Isaak Hernandez) - американский актер, ставший известным благодаря главной роли в картине братьев...</p>;
  const fullText = <p>Оскар Айзек (Oscar Isaak Hernandez) - американский актер, ставший известным благодаря главной роли в картине братьев Коэн «Внутри Льюина Дэвиса».</p>;
      ...
      <div className={styles.container}>
        ...
        <section className={styles.personContainer}>
          <div className={styles.imgContainer}>
            <Image src={person.personPicture} alt="person" className={styles.img} width={150} height={225} priority></Image>
          </div>
          <h1 className={styles.title}>{router.asPath.includes("lang=en") ? person.personLang[1].personName : person.personLang[0].personName}</h1>
          <div className={styles.title_en}>{router.asPath.includes("lang=en") ? person.personLang[0].personName : person.personLang[1].personName}</div>
...
export const getStaticProps: GetStaticProps = async (context) => {
  ...
  let person;
  try {
    const response = await axios.get(`http://84.201.131.92:5002/persons/${id}?lang=${lang}`, { timeout: 5000 });
